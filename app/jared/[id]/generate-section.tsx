...
export function JaredGenerate({ jobId, initialCover, initialResume }: Props) {
  return (
    <div className="mt-10 space-y-8">
      <Panel jobId={jobId} kind="cover_letter" label="Cover letter" initial={initialCover} />
      <Panel jobId={jobId} kind="tailored_resume" label="Tailored resume" initial={initialResume} />
    </div>
  );
}
  ...
  const generate = async (force: boolean) => {
    ...
    try {
      const res = await fetch(
        `/api/jared/generate/${jobId}?kind=${kind}${force ? '&force=1' : ''}`,
        { method: 'POST' },
      );
        ...
        <div className="doc-prose px-6 py-6 text-[15px] text-zinc-800 dark:text-zinc-200">
          <ReactMarkdown
            ...
            components={{
              ...
              ul: ({ children }) => <ul className="mt-2 list-disc space-y-1 pl-6 leading-relaxed">{children}</ul>,
