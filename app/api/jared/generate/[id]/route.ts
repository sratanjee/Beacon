...
// Public endpoint scoped to Jared's UID — matches app/jobs/[id]/generate but
// skips auth (this is a friend-share URL). No abuse mitigation beyond the
// existing generated_docs cache; if hit rate becomes an issue add a simple
...
export async function POST(
  ...
  if (kind !== 'cover_letter' && kind !== 'tailored_resume') {
    return NextResponse.json({ error: 'kind must be cover_letter or tailored_resume' }, { status: 400 });
  }
  ...
  let result;
  try {
    result = kind === 'cover_letter' ? await generateCoverLetter(ctx, profile) : await generateTailoredResume(ctx, profile);
  } catch (e) {
  ...
  const upsert = await db
    ...
    .single();
  if (upsert.error) {
    return NextResponse.json({ error: `save failed: ${upsert.error.message}`, text: result.text }, { status: 500 });
  }
